...
export async function resumeDependents(database: SqlQueryable, workId?: string) {
  await database.query(`WITH RECURSIVE blocked AS (
    SELECT dependency.work_id FROM lingxios.agent_work_dependencies dependency
      ...
      WHERE ($1::text IS NULL OR prerequisite.id=$1) AND (prerequisite.status IN ('partial','blocked','failed','cancelled') OR prerequisite.cancel_requested_at IS NOT NULL
    ...
    WHERE work.status='queued' AND work.id IN(SELECT work_id FROM blocked)`, [workId ?? null])
  return database.query(`WITH ready AS (
    ...
    WHERE ($1::text IS NULL OR parent.id=$1 OR parent.id=(SELECT meta->>'parentWorkId' FROM lingxios.agent_work_items WHERE id=$1))
      AND parent.status='waiting' AND parent.goal_outcome->>'status'='delegated' AND parent.cancel_requested_at IS NULL
    ...
    goal_outcome=NULL,updated_at=NOW() FROM ready WHERE parent.id=ready.id`, [workId ?? null])
}
