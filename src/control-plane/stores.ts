...
export interface WorkStore {
  ...
  hasChild(parent: Omit<WorkItem, 'leaseToken'>, childId: string, requestVersion: number): Promise<boolean>
  ...
  wait(id: string, fence: number, leaseTokenHash: string, outcome: import('../protocol/outcome.js').WaitingOutcome): Promise<boolean>
...
export interface ActionExecutor {
  ...
  prepare(work: Omit<WorkItem, 'leaseToken'>, action: import('../protocol/types.js').HostAction,
    options: ActionExecutionOptions): Promise<void>
  execute(work: Omit<WorkItem, 'leaseToken'>, action: import('../protocol/types.js').HostAction,
    options: ActionExecutionOptions): Promise<HostActionResult>
  ...
  verifyResult?(work: Omit<WorkItem, 'leaseToken'>, action: import('../protocol/types.js').HostAction,
    value: unknown, options: ActionExecutionOptions): Promise<Omit<import('../outcome/verification.js').VerificationRecord, 'checker'>>
  /** Package-owned, explicit read-only allowlist; never route unknown methods to execute. */
  readResource?(work: Omit<WorkItem, 'leaseToken'>, action: import('../protocol/types.js').HostAction,
    options: ActionExecutionOptions): Promise<unknown>
}
