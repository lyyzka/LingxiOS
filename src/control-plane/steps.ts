...
export class PgStepStore implements StepStore {
  ...
  async save(proof: StoreLeaseProof, step: ExecutionStep) {
    ...
    const { rows } = await this.database.query(`INSERT INTO lingxios.agent_steps
      (work_id,step_id,request_version,kind,input_hash,input,output,artifacts,completed_at)
      SELECT $1,$4,$5,$6,$7,$8::jsonb,$9::jsonb,$10::jsonb,CASE WHEN $9::jsonb IS NULL THEN NULL ELSE NOW() END
      ...
      ON CONFLICT(work_id,step_id) DO UPDATE SET output=EXCLUDED.output,artifacts=EXCLUDED.artifacts,completed_at=EXCLUDED.completed_at
      ...
      RETURNING step_id`, [proof.workId, proof.fence, proof.leaseTokenHash, step.id, step.requestVersion,
      step.kind, inputHash, JSON.stringify(step.input), step.output === undefined ? null : JSON.stringify(step.output), JSON.stringify(step.artifacts)])
