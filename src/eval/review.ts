...
export async function reviewAnswer(source: ModelDriver | ModelConfiguration, input: { originalInput: string; revisions: string[]; answer: string; rubric: string; observations?: readonly ResourceObservation[] }, signal?: AbortSignal) {
  ...
  const compiled = compileAuxiliaryPrompt('evaluation-review', instructions + observationInstructions)
  if (Buffer.byteLength(serialized + compiled.instructions, 'utf8') + reserve + 1024 > window) throw new Error('review input exceeds context budget; original input was not truncated')
  const started = Date.now()
  const result = await model.structured({ instructions: compiled.instructions, prompt: compiled.manifest, input: JSON.parse(serialized) as unknown, signal: signal ?? AbortSignal.timeout(60_000) })
