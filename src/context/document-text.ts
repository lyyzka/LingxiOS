...
let active = 0
export async function extractDocumentText(bytes: Uint8Array, format: 'docx' | 'pdf', signal?: AbortSignal): Promise<string> {
  ...
  try {
    return await new Promise<string>((resolve, reject) => {
      const child = execFile(process.execPath, ['--max-old-space-size=128', fileURLToPath(new URL('./document-worker.js', import.meta.url)), format],
        { timeout: 10_000, maxBuffer: 4 * 1024 * 1024, windowsHide: true, env: {}, ...(signal ? { signal } : {}) }, (error, stdout) => {
