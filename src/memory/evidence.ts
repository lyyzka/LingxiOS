...
export async function captureMemoryEvidence(database: SqlQueryable, work: Omit<WorkItem, 'leaseToken'>, message: AssistantMessage, scopes: readonly MemoryScope[]) {
  ...
  if (scopes.length > 12 || scopes.some(scope => scope.tenantId !== work.tenantId)
    || new Set(scopes.map(scope => scope.scopeType)).size !== scopes.length) throw new Error('invalid memory evidence scopes')
...
export async function retryMemorySynthesis(database: SqlQueryable) {
  ...
  await database.query(`WITH retry AS (
    ...
    WHERE w.kind IN ('memory_synthesis','memory_index','memory_evaluation') AND w.status='failed' AND w.attempts<3 AND w.cancel_requested_at IS NULL
      AND (w.kind='memory_index' OR (w.kind='memory_synthesis' AND e.status='pending') OR (w.kind='memory_evaluation' AND e.status='processed'))
