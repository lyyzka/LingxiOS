...
import { searchMemories, memoryDocument, memoryEntry, type MemoryScope } from './store.js'
import { excerpt, pageLimit } from './text.js'
import type { MemoryHit, MemorySearchResult } from './types.js'
...
export function createSemanticMemory(database: SqlPool, options: EmbeddingOptions, budget: RootModelBudgetOptions = {}) {
  ...
  return {
    async recall(work: Omit<WorkItem, 'leaseToken'>, scope: MemoryScope, query: string, limit: number, signal?: AbortSignal): Promise<MemorySearchResult> {
      signal?.throwIfAborted()
      pageLimit(limit)
      if (scope.tenantId !== work.tenantId || query.length > 2000) throw new Error('invalid memory recall scope or query')
      ...
      const enqueue = (model: string | null) => database.query(`INSERT INTO lingxios.agent_work_items
        ...
        SELECT 'memory-index:'||md5(jsonb_build_array($1::text,m.id,m.version,$4::text,$5::text,$6::text,$7::text,$8::text,$11::text)::text),
          ...
          jsonb_build_object('memoryId',m.id,'version',m.version,'modelKey',$4::text,'expectedModel',$11::text,'scopeType',m.scope_type,'scopeId',m.scope_id,'sourceWorkId',$9::text)
      ...
      if (!query.trim()) { await enqueue(null); return searchMemories(database,scope,'',limit) }
      const key = createHash('sha256').update(JSON.stringify([scope.tenantId, work.principalId, work.id, work.fence, query])).digest('hex')
      ...
      try { vector = await cached.value }
        ...
        return { ...await searchMemories(database,scope,query,limit),retrieval: 'keyword_embedding_unavailable' }
      }
      ...
      return withTransaction(database, async client => {
        ...
        const keyword=await searchMemories(client,scope,query,64)
        const { rows } = await client.query(`SELECT m.*,
          (SELECT SUM(a*b) FROM unnest(e.embedding,$6::double precision[]) AS v(a,b)) AS similarity
          FROM lingxios.agent_memories m JOIN lingxios.agent_memory_embeddings e
          ...
          WHERE m.tenant_id=$1 AND m.scope_type=$2 AND m.scope_id=$3 AND m.origin<>'evolved' AND m.status='active' AND (m.valid_until IS NULL OR m.valid_until>NOW())
          ORDER BY similarity DESC,m.path LIMIT $7`,
        [scope.tenantId, scope.scopeType, scope.scopeId, driver.cacheKey, vector.model, vector.vectors[0], 64])
        const ranks = new Map<string,MemoryHit>()
        const add = (items: MemoryHit[]) => items.forEach((item,index) => {
          const existing = ranks.get(item.id)
          ranks.set(item.id,{ ...item,score: (existing?.score ?? 0)+1/(60+index+1) })
        })
        add(keyword.items as MemoryHit[])
        add(rows.filter(row => Number(row['similarity'])>0).map(row => ({ ...memoryEntry(memoryDocument(row)),excerpt: excerpt(String(row['body']),1600),score: 0 })))
        return { items: [...ranks.values()].sort((a,b) => b.score-a.score || a.path.localeCompare(b.path)).slice(0,limit),
          nextCursor: null,retrieval: 'hybrid' as const }
      }).catch(async error => {
        signal?.throwIfAborted()
        if ((error as { code?: string }).code !== '57014') throw error
        return { ...await searchMemories(database,scope,query,limit),retrieval: 'keyword_embedding_unavailable' as const }
      })
    },
    async refresh(work: Omit<WorkItem, 'leaseToken'>, authorize: (database: SqlQueryable) => Promise<void>, signal?: AbortSignal) {
      ...
      const chunks: string[]=[]
      let remaining=String(initial['body'])
      while (remaining) { const chunk=excerpt(remaining,8000); chunks.push(chunk); remaining=remaining.slice(chunk.length) }
      const result = await embed(work,chunks,signal)
      const embedding=result.vectors[0]!.map((_,index)=>result.vectors.reduce((sum,vector)=>sum+vector[index]!,0)/result.vectors.length)
      const norm=Math.hypot(...embedding)
      if (!norm) throw new Error('memory embedding has zero magnitude')
      ...
      return withTransaction(database, async client => {
        ...
        [work.tenantId, current['id'], current['version'], driver.cacheKey, result.model, embedding.map(value=>value/norm)])
