...
export class ModelLectureAuthor implements LectureAuthor {
  ...
  async slide(input: Parameters<LectureAuthor['slide']>[0], signal?: AbortSignal): Promise<SlideSpec> {
    return this.generate(`Author one professional 1280x720 Chinese teaching slide. Return only this exact JSON shape: ${SLIDE_OUTPUT}. Draw one self-contained SVG with viewBox="0 0 1280 720" and role="img"; no style, script, event handler, foreignObject, external resource, gradient, shadow, glow, or 3D data marks. Reserve y=0..183 for the conclusion-style title and y=641..719 for the footer: every instructional component and anchor must stay inside x=40..1240 and y=184..640 so text never overlaps the SVG lesson objects.

Choose the visual form before color: magnitude=thin bar, trend=2px line, process=labeled arrows, comparison=aligned before/after, single value=large number instead of a fake chart. Depict the mechanism, not a row of named boxes. Use one axis only; no pie for close values, rainbow, thick blocks, heavy/dashed grids, border around data marks, or number on every point. Bars are at most 24px thick with 2px surface gaps; markers are at least 8px; axes/gridlines are quiet hairlines. Use at most three semantic hues, keep labels in neutral ink, label arrows, align objects to a grid, and use short labels that fit without clipping. Every SVG text element must have class t, ts, th, or tn.

For an instructional slide create 2-4 distinct, meaningful SVG components. Put data-anchor-id on the smallest complete <g> or SVG element that visually contains each explained component, never on the root <svg>. Give every explanation step a different primary anchorIds[0], with matching measured 1280x720 coordinates; do not reuse one whole-chart anchor across steps. Each explanation follows observation → reason → meaning and cites known claim IDs. A source or derived binding must copy snapshotId and evidenceMarkers exactly from the same input.evidence[] snapshot; never substitute sourceId or chunkId. Mark invented values as teaching-example.`, input,
...
export class ModelLectureReviewer implements LectureReviewer {
  ...
  async review(deck: DeckManifest, signal?: AbortSignal, progress?: LectureProgressObserver): Promise<ValidationReport> {
    ...
    for (let offset = 0; offset < deck.slides.length; offset += 10) {
      ...
      const result = await this.model.structured({ instructions: 'Independently review these lecture pages for citation support, teaching continuity, meaningful and correctly chosen chart forms, skipped prerequisites, label collisions, title/SVG overlap, clipped marks, dual axes, excessive labels or colors, and component-specific anchors. Reject any instructional page whose steps repeatedly target one whole-chart anchor instead of distinct explained SVG components. Return strict JSON {"passed":boolean,"issues":[{"code":"string","message":"string","pageId":"optional","objectId":"optional"}]}. Do not claim browser geometry was checked.', input: { course: deck.course, slides, evidence }, signal })
