...
export async function doctor(options: { database?: SqlQueryable; pythonCommand?: string; env?: NodeJS.ProcessEnv } = {}) {
  ...
  checks.push({ name: 'model_configuration', status: env['AGENT_OS_MODEL_API_KEY']?.trim() ? 'passed' : 'failed', detail: 'Checks credential configuration presence only; all generation tasks use the configured primary model. Provider access and quality are not tested.' })
  if (options.database) {
    try {
