...
export async function enqueueChild(database: SqlQueryable, parent: Omit<WorkItem, 'leaseToken'>, version: number | null, input: ChildInput) {
  ...
  if (!ancestors.rows.some(row => row['id'] === (parent.meta?.['rootWorkId'] ?? parent.id))
  ...
  return enqueueWork(database, { id: input.id, tenantId: parent.tenantId, agentId: input.agentId,
    sessionId: input.sessionId ?? parent.sessionId, ...(input.threadId ?? parent.threadId ? { threadId: input.threadId ?? parent.threadId! } : {}),
