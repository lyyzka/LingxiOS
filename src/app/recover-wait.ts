...
export async function recoverWait(database: SqlPool, service: ControlPlaneService, work: WorkItem): Promise<boolean> {
  ...
  const { rows } = await database.query(`SELECT intent.intent->'action' AS action,receipt.result
  ...
  if (!rows.length || rows.length > 100 || rows.some(row => !row['result'])) return false
