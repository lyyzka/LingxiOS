...
export const CALENDAR_METHODS = { list: ['from', 'to'], get: ['eventId'] } as const
...
export async function executeCalendar(db: SqlQueryable, services: Pick<LingxiLoopServices, 'calendar' | 'permissionService'>,
  ...
  if (method === 'get' && (typeof eventId !== 'string' || !eventId.trim() || eventId.length > 2000)) throw new Error('eventId is required')
  ...
  const authorization = { actorUserId: work.principalId, companyId: work.tenantId, projectId,
    action: 'calendar:read' as const, resource: method === 'get'
  ...
  const value = method === 'list' ? await api.calendarApplication.list(scope, range) : await api.calendarApplication.get(scope, eventId as string)
  await services.permissionService.assertCan(conversation)
  await services.permissionService.assertCan(authorization)
  if (Array.isArray(value)) return { events: value.slice(0, 100), truncated: value.length > 100 }
