...
`packageResources()` returns the schema-10 fresh-install schema, `migration008` for schema 7→8, `memoryReset009` for the explicit schema 8→9 memory reset, `migration010` for the additive schema 9→10 collaboration upgrade, optional `migration011` for schema-10 notification triggers and unchanged request-snapshot write suppression, and the Python runner. Apply SQL under the product's migration lock. Fresh installations use only `schema`; existing schema-9 installations first apply `migration010`, then may apply `migration011`. Older installations apply each intervening migration in order. Application startup performs read-only readiness checks and never executes package DDL. See [performance rollout](performance.md) for switches, pool sizing and validation limits.

Every public entry uses runtime version `3.2.1`, schema `10`, control-plane protocol `8`, Kernel protocol `2`, and assistant message `2`. Protocol 8 rejects workers without the IM isolation and collaboration context contract. Pin the exact npm version and verify the schema marker. Every generation, review, synthesis and compaction call uses the configured primary `model`. Optional embeddings remain a separate vector protocol.
