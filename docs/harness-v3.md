...
包版本为 3.0.0，schema 为 8，控制面协议为 6。协议升级会拒绝不支持模式约束的旧 Worker；Kernel 协议和已提交消息版本不变。升级和回滚步骤见 [packaged runtime](packaged-runtime.md)。迁移由宿主显式执行，本包启动只读检查，不自动改生产数据库或流量。
...
`memory.writePolicy` 对显式笔记、自动合成和进化候选共享生效，可允许、脱敏或拒绝；最终写入还拒绝明显凭证。产品负责配置领域隐私规则。工具应使用事务内的 `ActionContext.writeMemory`，低层 `writeMemory` 仅面向可信宿主；直接使用时应显式传入同一策略。

`forgetMemory` 在授权作用域内提升持久 epoch、清除记忆、历史版本和索引，并使旧证据失效。单条删除保守地使该作用域全部旧来源失效，其他记忆保留；旧任务即使重试、换 Worker 或尝试重新采集来源，也不能回写。作用域锁、epoch 核验和提交处于同一数据库事务。新请求可以形成新记忆。该接口处理长期记忆及合成证据；不会替产品删除会话、法定审计记录或历史模型 trace，后者仍按宿主保留/删除策略管理。
