...
it('bounds parser concurrency and rejects oversized input and expanded text', async () => {
  ...
  const pending = Array.from({ length: 18 }, () => extractDocumentText(Buffer.from('bad'), 'docx', stop.signal)
    .then(() => assert.fail('corrupt input accepted'), error => error))
  await assert.rejects(extractDocxText(Buffer.from('bad')), /resource queue is full/)
  stop.abort()
  assert.ok((await Promise.all(pending)).every(error => error instanceof Error))
